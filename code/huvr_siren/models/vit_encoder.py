...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        if self.num_registers and not self.keep_registers:
...
def vitb_32_norm_only_rope_fixed(**kwargs):
    model = VisionTransformer(
        patch_size=32,
        embed_dim=768,
        depth=12,
        ...
        naiveswiglu=True,
        rope=True,
        pt_hw_seq_len=8,    # training from scratch: 256/32=8 patches per side (no RoPE interpolation needed)
        intp_freq=True,
        pool_type="map",
        num_classes=0,
        **kwargs)
...
def vitb_8_norm_only_rope_fixed(**kwargs):
    model = VisionTransformer(
        patch_size=8,
        embed_dim=768,
        depth=12,
        ...
        naiveswiglu=True,
        rope=True,
        pt_hw_seq_len=32,   # training from scratch: 256/8=32 patches per side
        intp_freq=True,
        pool_type="map",
        num_classes=0,
        **kwargs)
