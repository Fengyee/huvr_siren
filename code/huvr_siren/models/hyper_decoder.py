### lightly adapted from https://github.com/yinboc/trans-inr/blob/master/models/transformer.py
### trans-inr is released under the BSD 3-Clause License
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, dim, depth, n_head, head_dim, ff_dim, dropout=0., attention=True, use_registers=False):
        super().__init__()
        assert not use_registers
        self.layers = nn.ModuleList()
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim, n_head, head_dim, dropout=dropout)) if attention else nn.Identity(),
